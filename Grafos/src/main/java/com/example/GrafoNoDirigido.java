...
public class GrafoNoDirigido {
    ...
    public Set<Comparable> findArticulationPoints() {
        time = 0;
        ...
        for (Vertice vertice : vertices.values()) {
            if (!visitedTimes.containsKey(vertice.getEtiqueta())) {
                findArticulationPoints(vertice, visitedTimes, lowTimes, parents);
            }
        }
    ...
    private void findArticulationPoints(Vertice vertice, Map<Comparable, Integer> visitedTimes, Map<Comparable, Integer> lowTimes, Map<Comparable, Comparable> parents) {
        ...
        for (Object obj : vertice.getAdyacentes()) {
            if (obj instanceof Adyacencia) {
                ...
                if (!visitedTimes.containsKey(child.getEtiqueta())) {
                    ...
                    childCount++;
                    findArticulationPoints(child, visitedTimes, lowTimes, parents);
    ...
    public Map<Comparable, Double> dijkstra(Comparable etiquetaOrigen) {
        ...
        PriorityQueue<VerticeDistancia> colaPrioridad = new PriorityQueue<>(Comparator.comparingDouble(VerticeDistancia::getDistancia));
        
        for (Comparable etiqueta : vertices.keySet()) {
            if (etiqueta.equals(etiquetaOrigen)) {
                distancias.put(etiqueta, 0.0);
                colaPrioridad.add(new VerticeDistancia(etiqueta, 0.0));
            } else {
                distancias.put(etiqueta, Double.POSITIVE_INFINITY);
                colaPrioridad.add(new VerticeDistancia(etiqueta, Double.POSITIVE_INFINITY));
            }
        ...
        while (!colaPrioridad.isEmpty()) {
            VerticeDistancia verticeActual = colaPrioridad.poll();
            Vertice vertice = vertices.get(verticeActual.getEtiqueta());

            for (Object obj : vertice.getAdyacentes()) {
                if (obj instanceof Adyacencia) {
                    Adyacencia adyacencia = (Adyacencia) obj;
                    Vertice vecino = adyacencia.getDestino();
                    double nuevaDistancia = distancias.get(vertice.getEtiqueta()) + adyacencia.getCosto();

                    if (nuevaDistancia < distancias.get(vecino.getEtiqueta())) {
                        distancias.put(vecino.getEtiqueta(), nuevaDistancia);
                        predecesores.put(vecino.getEtiqueta(), vertice.getEtiqueta());
                        colaPrioridad.add(new VerticeDistancia(vecino.getEtiqueta(), nuevaDistancia));
                    }
                }
            }
        }
    ...
    // Clase auxiliar para manejar las distancias en la cola de prioridad
    private class VerticeDistancia {
        private Comparable etiqueta;
        private double distancia;

        public VerticeDistancia(Comparable etiqueta, double distancia) {
