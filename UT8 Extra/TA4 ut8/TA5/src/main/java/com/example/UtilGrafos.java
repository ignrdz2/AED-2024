...
public class UtilGrafos {
    ...
    public static Double[][] obtenerMatrizCostos(Map<Comparable, Vertice> vertices) {
        ...
        while (i < cantidadVertices) {
            ...
            while (j < cantidadVertices) {
                Vertice elemVerticeI = vertices.get(VerticesIArr[i]);
                Vertice elemVerticeJ = vertices.get(VerticesJArr[j]);

                if (!elemVerticeI.getEtiqueta().equals(elemVerticeJ.getEtiqueta())) {
                    Vertice verticeI = (Vertice) elemVerticeI;
                    Vertice verticeJ = (Vertice) elemVerticeJ;
    ...
    public static void imprimirMatriz(Comparable[][] matriz, Map<Comparable, Vertice> vertices) {
    ...
    public static void imprimirMatrizCsv(Comparable[][] matriz, Map<Comparable, Vertice> vertices) {
        Object[] etiquetas = vertices.keySet().toArray();
    ...
    public static void imprimirMatrizMejorado(Comparable[][] matriz, Map<Comparable, Vertice> vertices, String titulo) {
    ...
    public static <T extends GrafoNoDirigido> T cargarGrafo(String nomArchVert, String nomArchAdy, 
        ...
        List<Vertice> verticesList = new ArrayList<Vertice>(vertices.length);
        List<Arista> aristasList = new ArrayList<Arista>(aristas.length);

        for (String verticeString : vertices) {
            if ((verticeString != null) && (verticeString.trim() != "")) {
                verticeString = verticeString.split(",")[0];
                verticesList.add(new Vertice(verticeString));
            }
        }
        for (String aristaString : aristas) {
            if ((aristaString != null) && (aristaString.trim() != "")) {
                String[] datos = aristaString.split(",");
                aristasList.add(new Arista(datos[0], datos[1], Double.parseDouble(datos[2])));
            }
        }
        try {
