...
public class TArbolBB<T> implements IArbolBB<T> {
    ...
    public boolean insertarBalanceado(Comparable etiqueta, T unDato) {
        TElementoAB<T> unElemento = new TElementoAB<>(etiqueta, unDato);
        if (raiz == null) {
            raiz = unElemento;
            return true;
    ...
    public void separar(LinkedList internos, LinkedList hojas) {
        if (!esVacio()) {
            raiz.separar(internos, hojas);
    ...
    // public List<T> obtenerNodosInternos() {
    //     List<T> nodosInternos = new LinkedList<>();
    //     if (!esVacio()) {
    //         raiz.obtenerNodosInternos(nodosInternos);
    //     }
